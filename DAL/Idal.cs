...
namespace DAL
{
    ...
    {
        void add_tester(int _id, string _lastname, string _firstname, DateTime _date_of_birth, gender _Gender, long _phone, Address _address, float _expirence, int _max_testPerWeek, vehicle _tester_expertise,bool [,] _work_time,int _max_way);// add teste
        ...
        bool id_alredy_exsits(int _id);
        bool id_tests_exsits(int _id);
        List<Tester> all_tester();
        List<Trainee> all_trainee();
        List<Test> all_test();
        Tester tester_by_id(int id);
        Trainee trainee_by_id(int id);
    }
}
